...
Usage: import the module (see Jupyter notebooks for examples), or run from
    ...
    python3 train_vanitas.py splash --weights=last --video=<URL or path to file>
"""
...
class SemioticsDataset(utils.Dataset):

    def load_semiotics(self, dataset_dir, subset):
        """Load a subset of the semiotics dataset.
        dataset_dir: Root directory of the dataset.
        subset: Subset to load: train or val
        """
        # Add classes.
        self.add_class("semiotics", 1, "key")
        self.add_class("semiotics", 2, "cross")
        self.add_class("semiotics", 3, "winged_lion")
    ...
    def load_mask(self, image_id):
       ...
       Returns:
        ...
        if image_info["source"] != "semiotics":
...
def train(model):
    """Train the model."""
    # Training dataset.
    dataset_train = SemioticsDataset()
    dataset_train.load_semiotics(args.dataset, "train")
    dataset_train.prepare()

    # Validation dataset
    dataset_val = SemioticsDataset()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset', required=False,
                        metavar="/path/to/semiotics/dataset/",
                        help='Directory of the semiotics dataset')
    ...
    if args.command == "train":
        ...
        class InferenceConfig(SemioticsConfig):
